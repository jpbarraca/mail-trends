...
class MailBoxPackageInfo(object):
  ...
  def __init__(self, path):
	...
	for dirname, dirnames, filenames in os.walk(self.path):
	  ...
	  for filename in filenames:
		if not "EC95A0DD-2A5B-48E9-8409-C590E46DC2E3" in dirname :
		  continue

		if not "Data" in dirname :
		  continue

		if not "Messages" in subdirname:
		  continue
		...
		if not filename.startswith('.') and filename.endswith(".emlx") and box in self.mailboxes:
		  #logging.info("Adding file %s/%s to box %s",dirname,filename,box)
		  path = os.path.join(dirname, filename )
		  self.mailboxes[box].append(path)
